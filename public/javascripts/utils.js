      ...
      `Error fetching ${route} with options: ${
        options ? JSON.stringify(options) : options
            ...
            Response wasn't json: ${
              responseText ? JSON.stringify(responseText) : responseText
            }
            error: ${getTextError}`
    );
  }
...
async function displayError() {
  document.getElementById("errorInfo").innerText =
    "Error: action failed (see console for more information)";
...
//               postInfo.username
//             )}">${escapeHTML(postInfo.username)}</a></div>
//         </div>`;
//     })
//     .join("\n");
  ...
  for (let i = 0; i < postsJson.length; i++) {
    ...
    let result = `<div class="post">
        ...
        <div>Username: <a href="/userInfo.html?user=${encodeURIComponent(
        postInfo.username
        )}">${escapeHTML(postInfo.username)}</a></div>`;
